% preamble.tex - the house preamble for every Portfolio explainer document.
%
% SOURCE OF TRUTH: .claude/skills/explain-project/preamble.tex. The build copies this
% file into <project>/docs/explainers/preamble.tex on every run, so it is committed
% beside the documents (a clone can rebuild them) and a change here reaches all 42
% projects the next time each is built. Do not edit the copy inside a project; it is
% overwritten. Both documents start with \input{preamble}.
%
% Adopted 2026-07-16 from the Digitalise Agency explainer skill, whose package notes
% below were verified against THIS machine's TeX Live on the same day.
...
% Kill hyphen ligatures in TYPEWRITER text. VERIFIED TRAP (2026-07-16, by rendering the
% page and reading the glyphs, not by reasoning about it): under T1, \texttt{--verbose}
% silently renders as "-verbose" and \texttt{---} as "--". A document could therefore
% document a CLI flag as the WRONG flag, or a Morse pattern as the wrong pattern, and
% look perfectly fine doing it. lstlisting and \verb were always safe; this makes \code,
% \file and \texttt safe too, which is what the prose actually uses.
% Prose is unaffected and still ligates: "a---b" in prose is an em-dash and stays banned.
...
\lstdefinestyle{house}{
  ...
  % reads bytes, so inputenc dies with "Invalid UTF-8 byte sequence" and the whole
  % document fails. Found 2026-07-16 by a real build hitting the Spanish punctuation in
  % morse-code-converter's alphabet table. These are the characters likely to turn up in
  % THIS portfolio's source (Latin-1 text, smart quotes a source file picked up from a
  % word processor, the degree and section signs).
  % NOT COVERED, and it cannot be: Urdu, Arabic and Balochi script. pdflatex has no font
  % for them. Do not paste them into a listing; see SKILL.md, "Non-Latin script".
  literate=%
